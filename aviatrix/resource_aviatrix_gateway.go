...
func resourceAviatrixGatewayRead(d *schema.ResourceData, meta interface{}) error {
	...
	if gw.CloudType == goaviatrix.AZURE && (isImport || zoneIsSet) && gw.GatewayZone != "AvailabilitySet" {
	...
	if ok && gw.CloudType == goaviatrix.AZURE {
	...
	if !gw.IsPsfGateway {
		...
		if gw.HaGw.GwSize == "" {
			...
			d.Set("peering_ha_subnet", gw.PsfDetails.GwSubnetCidr)
			d.Set("peering_ha_zone", gw.PsfDetails.GwSubnetAz)
		}
	}
	...
	} else if gw.HaGw.CloudType == goaviatrix.AZURE {
		d.Set("peering_ha_subnet", gw.HaGw.VpcNet)
